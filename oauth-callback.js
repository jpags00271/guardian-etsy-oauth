 ...
 * Exchanges code for access token using stored code verifier
 */

import axios from 'axios';

// In-memory store for verifiers (state -> verifier mapping)
// In production, use a real database or cache
const verifierStore = {};
...
export default async function handler(req, res) {
  ...
  if (code && state) {
    try {
      ...
      const codeVerifier = verifierStore[state];

      if (!codeVerifier) {
        return res.status(400).json({
          success: false,
          error: 'Code verifier not found',
          message: 'The session or verifier has expired. Please restart the OAuth flow.',
          timestamp: new Date().toISOString()
        });
      }
...
// Export verifier store for use in other endpoints
